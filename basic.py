...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if sensing_info.speed < 140:  # ref_idx = 열 개의 지점 중 몇 번째 지점을 기준으로 핸들을 돌릴 건지 정하는 변수.
            ref_idx = 7  # 처음에 속도가 너무 느릴 때는 너무 짧게 보길래 강제로 멀리 보게 변수 조정.
        elif sensing_info.speed < 173:
            ref_idx = max(5, int(sensing_info.speed / refnum)+1)  # 속도 120km 이상일 때는 속도에 맞춰서 보도록.
        else:
        ...
        spdnum = abs(sensing_info.to_middle - 12) / 40 + max(0, abs(sensing_info.track_forward_angles[ref_idx]) - abs(
            sensing_info.moving_angle / 2) - 4) / 600 + max(0, sensing_info.speed - 100) / 105
        # steering = 핸들 꺽는 정도.
        ...
        brk = max(0, abs(sensing_info.track_forward_angles[ref_idx] - sensing_info.moving_angle) - 25) / 840 + max(0,
                                                                                                                   sensing_info.speed - 180) / 100
        # 속도가 200이 넘으면 brk에 0.3을 더해 더 세게 브레이크 잡기.
        if sensing_info.speed < 163:
            car_controls.throttle = 1
        elif sensing_info.speed < 174:
            car_controls.throttle = 0.97
        elif sensing_info.speed < 190:
            car_controls.throttle = 0.95
            car_controls.brake = 0.02 + brk
        elif 190 <= sensing_info.speed:
            car_controls.throttle = 0.8
            car_controls.brake = 0.05 + brk
