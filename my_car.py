...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if sensing_info.speed < 150: set_throttle = 0.9  ## 속도가 60Km/h 이하인 경우 0.9 로 설정
        if sensing_info.speed > 180: set_throttle = 0.6  ## 최대속도를 80km/h로 설정

        ## 차량의 Speed 에 따라서 핸들을 돌리는 값을 조정함
        steer_factor = sensing_info.speed * 1
        if sensing_info.speed > 100: steer_factor = sensing_info.speed * 0.8
        if sensing_info.speed > 150: steer_factor = sensing_info.speed * 0.7
        steer_factor += abs(ref_angle - sensing_info.moving_angle)/2
